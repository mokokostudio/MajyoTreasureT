...
export function Sk1010(_container, _dpr){
  ...
  this.do = function(_atker, _beAtker){
    ...
    let doDraw = (() =>{
      ...
      if(curDate - lastDate >= diffTime){
        ...
        let that = this
        let img = new Image()
        img.src = require('@/assets/frameimg/skills/1010/cast_' + idx + '.png')
        img.onload = (() => {
          that.cav.clearRect(0, 0, that.element.width, that.element.height)
          that.cav.drawImage(img, _beAtker.xStand - 40, _beAtker.yStand - 100, 200, 253)
        })
        idx++
        if(idx === 49){
          that.skillStatus = 99
