...
var (
	TimeZone_GMT, _      = time.LoadLocation("GMT")
	TimeZone_ShangHai, _ = time.LoadLocation("Asia/ShangHai")
)
...
func IsTimestampInSameDayInLocation(a, b int64, location *time.Location) bool {
	timeA := time.Unix(a, 0).In(location)
	timeB := time.Unix(b, 0).In(location)
	ya, ma, da := timeA.Date()
	yb, mb, db := timeB.Date()
	return da == db && ya == yb && ma == mb
}
