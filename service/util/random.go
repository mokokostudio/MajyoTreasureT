...
func ReadWeightRandomPoolFromString(poolStr string) ([]*mpb.RandomNode, error) {
	if len(poolStr) == 0 {
		return nil, nil
	}
	var ret = make([]*mpb.RandomNode, 0)
	for _, str := range strings.Split(poolStr, ";") {
		strs := strings.Split(str, ":")
		if len(strs) != 2 {
			return nil, errRandomPoolString
		}
		ret = append(ret, &mpb.RandomNode{
			Id:     gutil.StrToUint32(strs[0]),
			Weight: gutil.StrToUint32(strs[1]),
		})
	}
	return ret, nil
}
