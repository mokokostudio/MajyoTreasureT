...
// Handler HTTPErrorHandler for decoration
func (eh *HTTPErrorHandler) APIHandler(f HTTPErrorHandlerFunc) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		err := f.ServeHTTP(w, r)
		if err != nil {
			// grpc unknown error and other type of error
			desc := grpc.ErrorDesc(err)
			eh.logger.Info(r.RequestURI, zap.String("error desc", desc))
			//http.Error(w, desc, http.StatusOK)
		}
	})
}
...
func ReadHTTPJSONReq(w http.ResponseWriter, r *http.Request, i interface{}, options HTTPEncryptionOptions) error {
	...
	if err != nil {
		return &AppError{err, "invalid request body, json umarshal failed", http.StatusBadRequest} //nolint:goerr113
	}
	return nil
}
...
func HttpsPost(ctx context.Context, reqUrl string, headers map[string]string, data []byte) ([]byte, error) {
