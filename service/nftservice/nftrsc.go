...
func (rm *NFTResourceMgr) loadNFTCollection(csvPath string) error {
	...
	for _, data := range datas {
		node := &mpb.NFTCollectionRsc{
			NftType: mpb.ENFT_NFTType(gcsv.StrToInt32(data["nfttype"])),
		}
...
func (rm *NFTResourceMgr) loadNFTConfig(csvPath string) error {
	...
	c := &mpb.NFTConfigRsc{
		NftGraphiqlQueryLimit:  gcsv.StrToUint64(datas[0]["nftgraphiqlquerylimit"]),
		NftGraphiqlPageNum:     gcsv.StrToUint64(datas[0]["nftgraphiqlpagenum"]),
